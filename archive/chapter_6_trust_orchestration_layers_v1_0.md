...
**Chapter Length:** ~12,000 words (24 pages)  
**Version:** 1.0 (Initial Draft)  
**Date:** November 26, 2025

---

## PART 1: TRUST + ORCHESTRATION ARCHITECTURE INTRODUCTION
...
graph TB
    ...
    BUILD["<b>🔧 Chapter 6 Builds Here</b><br/><b>Layers 5-6-7</b><br/><b>Trust + Orchestration</b>"]
...
| Week | Layer | Milestone |
|------|-------|-----------|
| Week 8 | Layer 5: Governance | ABAC policies deployed, HITL workflows operational |
| Week 9 | Layer 6: Observability | Distributed tracing, LLM cost visibility |
| Week 10 | Layer 7: Orchestration | Multi-agent coordination, architecture complete |

**Diagram 2: 7-Layer Agent-Ready Architecture—Trust + Orchestration Highlighted**

```mermaid
graph TB
    subgraph "<b>TRUST + ORCHESTRATION</b>"
...
| Dimension | Week 7 | Week 10 | Change |
|-----------|--------|---------|--------|
| I (Instant) | 5/6 | 5/6 | — |
| N (Natural) | 5/6 | 5/6 | — |
| P (Permitted) | 2/6 | 6/6 | +4 |
| A (Adaptive) | 5/6 | 5/6 | — |
| C (Contextual) | 5/6 | 5/6 | — |
| T (Transparent) | 3/6 | 6/6 | +3 |
| **Total** | **67/100** | **85/100** | **+18** |
...
Echo entered Week 8 with intelligence operational but constrained:

**What Was Working:**
- Sub-2-second query responses
- 95.6% accuracy on clinical queries
- 150,000 documents indexed in RAG
- 85% cache hit rate reducing costs
- 2,400 clinical terms semantically resolved

**What Was Missing:**
- No dynamic permissions—only static RBAC from Epic
- No HITL for high-risk decisions (Warfarin-class medications had no escalation)
- No cost visibility ($26K/month LLM spend with no breakdown)
- No distributed tracing (debugging meant log file archaeology)
- No multi-agent coordination (complex queries required manual decomposition)

Sarah's mandate was clear: complete the architecture in three weeks.
...
Marcus spoke first. "Governance has to come before anything else. We can't deploy clinical agents without dynamic authorization. The compliance team has been clear—static RBAC isn't sufficient for PHI access in agent contexts. HIPAA requires reasonable and appropriate access controls, and 'appropriate' means contextual in 2025."
...
Echo's pre-transformation authorization relied entirely on Epic's native RBAC. Physicians could access any patient record. Nurses could view but not modify orders. Administrators had department-scoped access. These static assignments couldn't evaluate context.

The limitations of RBAC became apparent through several scenarios that emerged during pilot testing:
...
The two-point improvement reflects the shift from static RBAC to dynamic ABAC:
...
Combined, these capabilities enable agents to operate in clinical contexts where static authorization would either over-permit (allowing risky access) or under-permit (blocking legitimate use). Dynamic governance finds the appropriate middle ground.
...
With Layer 6 operational, Echo gained unprecedented visibility into agent operations.

**Request Tracing Example:**
A clinical query about medication adherence now generates a complete trace:
- Query received: T+0ms
- Semantic parsing: T+12ms
- ABAC evaluation: T+15ms
- Vector search: T+45ms
- Document retrieval: T+89ms
- Context assembly: T+102ms
- LLM generation: T+1,340ms
- Response delivery: T+1,356ms

When the same query type shows T+3,200ms latency, engineers immediately see that LLM generation took 2,890ms—indicating an API slowdown or model congestion, not an infrastructure problem.
...
Monday through Friday, the governance build dominated.

Marcus Williams led policy development, working with Echo's compliance team to translate regulatory requirements into OPA rules. 247 policies emerged from sessions that often felt like contract negotiations—clinical operations wanted flexibility, compliance wanted constraints, and the architecture needed both.

The policy categories reflected healthcare's regulatory complexity:

**Patient Data Access (73 rules):** Who can access patient records, under what circumstances, with what audit requirements. The policies distinguished between a physician's own patients, emergency access to any patient, and research access to de-identified cohorts.

**Medication Queries (52 rules):** Risk stratification based on drug classification. Narrow therapeutic index drugs (Warfarin, lithium, digoxin) always escalate. Controlled substances require additional verification. Standard medications auto-approve with audit logging.

**Scheduling Operations (41 rules):** Appointment creation, modification, and cancellation policies. Patients can modify their own appointments. Staff can modify appointments within their department. Cross-department scheduling requires approval from receiving department.

**Financial Data (38 rules):** Insurance information, billing details, collection status. More restrictive than clinical data—financial information requires explicit need-to-know, not just role-based access.

**Administrative Functions (43 rules):** User management, system configuration, bulk operations. Most restrictive category—administrative changes require explicit authorization and generate immediate alerts.

"The breakthrough was risk scoring," Marcus explained during the Week 8 review. "Instead of binary allow/deny, we evaluate risk on a 0-10 scale. Low-risk operations (0-3) auto-approve. Medium-risk operations (4-6) log with enhanced audit. High-risk operations (7-10) escalate to HITL. Same data access—different treatment based on context."

Jamie Rodriguez deployed the OPA infrastructure alongside existing agent services. The sidecar pattern—OPA running as a companion container to each agent instance—eliminated network latency from policy evaluation. Queries added 8-12ms for policy checks, invisible against 1,800ms average total latency.
...
By Friday, 193 of 247 policies were tested and deployed. The remaining 54 covered edge cases requiring additional compliance review—situations like emergency override procedures and audit access for external regulators.

### Week 9: Observability Operational

The observability build proceeded faster than planned.

Echo's Layer 4 implementation had already introduced basic OpenTelemetry tracing for RAG pipeline debugging. Extending that instrumentation to all seven layers required consistent patterns, not greenfield development. By Wednesday, trace completeness exceeded 98%.
...
Datadog dashboards came together Thursday. Jamie designed three primary views:

**Operations Dashboard:** Real-time latency percentiles, error rates, and throughput. The 3 AM page would reference this dashboard. Key metrics: P50/P95/P99 latency, error rate by type, requests per minute by endpoint.

**Cost Dashboard:** LLM spend by model, by query type, by time period. Finance would monitor this weekly. Key metrics: daily spend trend, cost per query by category, model selection distribution.

**Quality Dashboard:** Accuracy metrics, cache hit rates, HITL patterns. Clinical leadership would review this monthly. Key metrics: response accuracy (sampled), cache utilization, HITL escalation rate and resolution time.
...
The trace for that escalation told the complete story:
...
Monday began the orchestration build.

The three specialized agents had been in development since Week 8—parallel work while governance and observability deployed. Week 10 was integration: connecting agents to the LangGraph supervisor, implementing shared state, testing coordination patterns.

**Care Coordination Agent** development focused on Epic scheduling APIs. The agent could check provider availability, create appointments, and send patient notifications. Integration challenges included rate limit handling (Epic throttles scheduling API calls) and time zone normalization (appointments stored in UTC, displayed in local time).

**Clinical Documentation Agent** development focused on EHR integration. The agent could retrieve patient summaries, medication lists, lab results, and clinical notes. The challenge was context windowing—some patient histories exceeded LLM context limits, requiring intelligent summarization.

**Revenue Cycle Agent** development focused on payer integration. Insurance verification, prior authorization status, and coverage determination required connecting to multiple payer portals with different APIs and response times. The complexity of healthcare financial data exchange reflects broader interoperability challenges documented by ONC.[18]
...
Wednesday through Thursday was testing. Multi-domain queries exercised every coordination pattern: single-agent routing, parallel execution, sequential processing, HITL escalation within orchestrated flows. Each pattern worked. Each edge case was documented.

The test matrix covered 47 scenarios:
- 12 single-domain queries (4 per agent)
- 18 dual-domain queries (all combinations)
- 8 triple-domain queries (various complexity levels)
- 6 error handling scenarios (agent timeout, partial failure, invalid input)
- 3 HITL integration scenarios (escalation within orchestrated flows)

All 47 scenarios passed by Thursday evening.
...
```mermaid
gantt
    title Echo's Trust + Orchestration Build (Weeks 8-10)
...
The journey had started with a simple question: Why do 95% of agent projects fail? The answer, as it turned out, was trust. Not LLM capability—the models were capable. Not data availability—the data existed. Trust. The infrastructure gap between what agents could theoretically do and what organizations could safely let them do.

Echo closed that gap. Layer by layer, week by week, capability by capability.
...
**Foundation (Layers 1-2, Weeks 1-4):**
- Multi-modal storage across 8 categories
- Real-time data fabric with 28-second freshness
- Investment: $470,000
- INPACT™: 28/100 → 42/100

**Intelligence (Layers 3-4, Weeks 5-7):**
- Semantic layer with 2,400 clinical terms
- RAG pipeline with 7 stages and 85% cache hit
- Investment: $380,000
- INPACT™: 42/100 → 67/100

**Trust + Orchestration (Layers 5-6-7, Weeks 8-10):**
- Governance with 247 ABAC policies and HITL workflows
- Observability with distributed tracing and cost visibility
- Orchestration with three specialized agents
- Investment: $82,000
- INPACT™: 67/100 → 85/100

**Total Investment:** $932,000 of $1.23M authorized  
**Total Timeline:** 70 days (10 weeks)  
**INPACT™ Improvement:** 57 points (28 → 85)

### All Seven Gaps Closed

| Gap | Infrastructure Need | Layer | Status |
|-----|---------------------|-------|--------|
| Gap 1 | Multi-Modal Storage | Layer 1 | ✓ Closed Week 4 |
| Gap 2 | Real-Time Data | Layer 2 | ✓ Closed Week 4 |
| Gap 3 | Semantic Understanding | Layer 3 | ✓ Closed Week 7 |
| Gap 4 | Intelligent Retrieval | Layer 4 | ✓ Closed Week 7 |
| Gap 5 | Dynamic Permissions | Layer 5 | ✓ Closed Week 9 |
| Gap 6 | Reasoning Observability | Layer 6 | ✓ Closed Week 9 |
| Gap 7 | Multi-Agent Coordination | Layer 7 | ✓ Closed Week 10 |

The gaps identified in Chapter 3 are resolved. The architecture is complete.
...
4. **The 7-Layer Architecture is complete:** Layers 1-2 (Foundation) provide data availability and freshness. Layers 3-4 (Intelligence) provide understanding and reasoning. Layers 5-6-7 (Trust + Orchestration) provide safety, visibility, and coordination. Together, they create production-ready agent infrastructure.
...
**Trust + Orchestration Phase (Weeks 8-10):**
- Gap 5 (Dynamic Permissions): From static RBAC to contextual ABAC with 247 policies
...
[7] Redis. (2024). "Redis Documentation." https://redis.io/documentation
